...
def can_skip_mamba_postprocess(
    ...
    """Return True iff mamba postprocess is provably a no-op this step.

    Bounded by ``n_draft + 1`` accepted tokens, we can decide on CPU
    whether any request can cross a mamba block boundary. If not, the
    caller can skip the fused postprocess kernel and defer the
    device-to-host sync of ``num_accepted_tokens``.

    Must stay in lockstep with the boundary-crossing logic in
    :func:`postprocess_mamba_fused_kernel` and :func:`preprocess_mamba`.
    """
